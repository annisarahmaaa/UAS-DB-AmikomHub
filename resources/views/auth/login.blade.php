    ...
    <div class="max-w-md w-full glow-card-wrapper z-10 transition-all duration-500 hover:scale-[1.01]">
        ...
        <div class="futuristic-card rounded-[2.4rem] p-8 md:p-10">
            ...
            <div class="flex bg-slate-100/50 p-1 rounded-2xl mb-6">
                <button type="button" onclick="switchTab('login')" id="tab-login" class="flex-1 py-2.5 text-[13px] font-bold rounded-xl bg-white text-indigo-600 shadow-sm transition-all duration-300">Login</button>
                <button type="button" onclick="switchTab('register')" id="tab-register" class="flex-1 py-2.5 text-[13px] font-bold rounded-xl text-slate-500 hover:text-slate-700 transition-all duration-300">Register</button>
            </div>

            <form action="{{ route('admin.login.post') }}" method="POST" class="space-y-5" id="form-login">
                @csrf
            ...
            <form action="{{ route('admin.register.post') }}" method="POST" class="space-y-5 hidden" id="form-register">
                @csrf
                
                <div class="space-y-1.5">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider pl-1">Nama Lengkap</label>
                ...
                <div class="space-y-1.5">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider pl-1">Mendaftar Sebagai</label>
                    <div class="grid grid-cols-2 gap-3">
                        <label class="relative cursor-pointer">
                            <input type="radio" name="role" value="user" class="peer sr-only" checked>
                            <div class="w-full px-4 py-3 bg-white/50 border-2 border-slate-200/80 rounded-2xl peer-checked:border-indigo-600 peer-checked:bg-indigo-50 flex items-center justify-center transition-all duration-200 shadow-sm">
                                <span class="font-bold text-[13px] text-slate-500 peer-checked:text-indigo-700">Pembeli Tiket</span>
                            </div>
                        </label>
                        <label class="relative cursor-pointer">
                            <input type="radio" name="role" value="organizer" class="peer sr-only">
                            <div class="w-full px-4 py-3 bg-white/50 border-2 border-slate-200/80 rounded-2xl peer-checked:border-indigo-600 peer-checked:bg-indigo-50 flex items-center justify-center transition-all duration-200 shadow-sm">
                                <span class="font-bold text-[13px] text-slate-500 peer-checked:text-indigo-700">Pengelola Event</span>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <!-- --- BAGIAN TOMBOL REGISTER SOSIAL (GOOGLE) --- -->
                <div class="pt-2">
                ...
                <div class="relative flex py-1 items-center">
                    <div class="flex-grow border-t border-slate-200"></div>
                    <span class="flex-shrink mx-4 text-[10px] uppercase tracking-widest font-bold text-slate-400">Atau Manual</span>
                    <div class="flex-grow border-t border-slate-200"></div>
                </div>
                <!-- -------------------------------------------- -->
    ...
    <script>
        function switchTab(tab) {
            ...
            if (tab === 'login') {
                ...
                tabLogin.classList.remove('text-slate-500', 'hover:text-slate-700');
                
                tabRegister.classList.remove('bg-white', 'text-indigo-600', 'shadow-sm');
                tabRegister.classList.add('text-slate-500', 'hover:text-slate-700');
            } else {
                formRegister.classList.remove('hidden');
                formLogin.classList.add('hidden');
                
                tabRegister.classList.add('bg-white', 'text-indigo-600', 'shadow-sm');
                tabRegister.classList.remove('text-slate-500', 'hover:text-slate-700');
                
                tabLogin.classList.remove('bg-white', 'text-indigo-600', 'shadow-sm');
                tabLogin.classList.add('text-slate-500', 'hover:text-slate-700');
            }
        }

        function registerWithGoogle() {
            const role = document.querySelector('input[name="role"]:checked').value;
            window.location.href = "{{ route('google.login') }}?role=" + role;
        }
