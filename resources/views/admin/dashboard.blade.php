                ...
                @forelse($recentTransactions as $trx)
                    <tr class="hover:bg-slate-50 transition duration-150">
                        <td class="px-8 py-6 text-sm text-slate-600 max-w-xs break-all">
                            <span class="font-medium text-slate-700">{{ $trx->created_at->format('d M y - H:i') }}</span>
                            <br>
                        ...
                        <td class="px-8 py-6">
                            ...
                            <p class="text-xs text-slate-400 truncate max-w-[150px] mt-0.5">{{ $trx->customer_email }}</p>
                        </td>

                        <td class="px-8 py-6 font-semibold text-slate-600 max-w-xs truncate">
                            {{ $trx->event->title ?? '-' }}
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-8 py-12 text-center text-slate-400 font-medium">
                            Belum ada riwayat transaksi yang tersimpan di database.
                        </td>
                    </tr>
                @endforelse
            </tbody>
